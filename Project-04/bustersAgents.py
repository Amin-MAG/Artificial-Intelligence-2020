...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        minimum_position = min([
            (belief.argMax(), self.distancer.getDistance(belief.argMax(), pacmanPosition))
            for belief in livingGhostPositionDistributions
        ], key=lambda x: x[1])[0]

        # find the best action we can perform
        return min([
            (act, self.distancer.getDistance(Actions.getSuccessor(pacmanPosition, act), minimum_position))
            for act in legal
        ], key=lambda x: x[1])[0]
